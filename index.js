...
class CoordinatePlane {
  ...
  constructor(_config = {unit: 30,
    ...
    // put a point (x,y)
    const origin = { x: this.xmax / 2, y: this.xmax / 2 };
  ...
  transformPt(pt) {
    return new Point(origin.x + pt.x * this.unit, origin.y - pt.y * this.unit);
  }
  ...
  drawCircle(c, config) {
    ...
    const drawCir = function (x, y, r, from, to, color, counterClockwise) {
      ...
      this.ctx.arc(c.center.x, c.center.y, c.radius, from, to, counterClockwise);
...
class Circle {
  ...
  prtEqn() {
    ...
    return `(x${plus(this.center.x)})^2 + (y${plus(this.center.y)})^2 = ${this.radius ** 2
      }`;
  }
}
